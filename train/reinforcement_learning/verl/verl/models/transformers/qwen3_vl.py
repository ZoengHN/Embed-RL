...
def _get_input_embeds(
    ...
    if pixel_values is None and pixel_values_videos is None:
        ...
        # NOTE: Use non-inplace operation to avoid "leaf Variable that requires grad" error in LoRA training
        # When using LoRA with frozen vision tower, inputs_embeds is a leaf variable that requires grad.
        # In-place operations (+=) on such variables will cause RuntimeError.
        # The "+ 0.0 * image_embeds.mean()" trick is to keep gradient graph connected even when vision is frozen.
