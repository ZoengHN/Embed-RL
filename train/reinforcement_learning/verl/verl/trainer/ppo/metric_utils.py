...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    # Add reward extra info metrics (e.g., format_reward, retrieval_accuracy, etc.)
    # These are added by custom reward functions via reward_extra_info
    reward_extra_keys = [
        "format_reward",
        "retrieval_accuracy",
        "similarity_margin",
        "pos_similarity",
        "rerank_accuracy_binary",
        "accuracy_x_margin",
        "rerank_acc_x_similarity",
        "retrieval_reward_unweighted",
        "rerank_reward_unweighted",
    ]
    for key in reward_extra_keys:
        if key in batch.non_tensor_batch:
            values = batch.non_tensor_batch[key]
            if isinstance(values, np.ndarray) and values.size > 0:
                metrics[f"reward/{key}/mean"] = float(np.mean(values))
                metrics[f"reward/{key}/max"] = float(np.max(values))
                metrics[f"reward/{key}/min"] = float(np.min(values))
