...
class RayPPOTrainer:
    ...
    def _create_dataloader(self, train_dataset, val_dataset, collate_fn, train_sampler: Optional[Sampler]):
        ...
        # Check if train_sampler is a batch_sampler (e.g., DatasetGroupedBatchSampler)
        # Batch samplers are incompatible with batch_size parameter
        from torch.utils.data import BatchSampler
        is_batch_sampler = isinstance(train_sampler, BatchSampler) or (
            train_sampler is not None and hasattr(train_sampler, '__iter__') and
            not hasattr(train_sampler, '__len__') == False and
            hasattr(train_sampler, 'batch_size')
        )
    ...
    def _save_rollout_visualization(self, gen_batch_output, gen_batch, step):
        """Save first sample's rollout results to txt file for visualization.

        Args:
            gen_batch_output: Generated batch output containing responses
            gen_batch: Original generation batch containing prompts
            step: Current training step
        """
        try:
            # Use dedicated rollouts directory
            output_dir = "/ytech_m2v_hdd/jianghaonan03/code/VR/verl/rollouts"
            os.makedirs(output_dir, exist_ok=True)

            # Get number of rollouts per sample (n parameter)
            ...
            if "data_source" in gen_batch.non_tensor_batch:
                data_source = gen_batch.non_tensor_batch["data_source"][0]

            if "query_base_text" in gen_batch.non_tensor_batch:
                query_base_text = gen_batch.non_tensor_batch["query_base_text"][0] or "N/A"

            if "sample_type" in gen_batch.non_tensor_batch:
                sample_type = gen_batch.non_tensor_batch["sample_type"][0]

            # Extract unique identifiers if available
            if "video_id" in gen_batch.non_tensor_batch:
                video_id = gen_batch.non_tensor_batch["video_id"][0]
            if "image_id" in gen_batch.non_tensor_batch:
                image_id = gen_batch.non_tensor_batch["image_id"][0]
            if "pos_video_id" in gen_batch.non_tensor_batch:
                pos_video_id = gen_batch.non_tensor_batch["pos_video_id"][0]
            if "pos_image_id" in gen_batch.non_tensor_batch:
                pos_image_id = gen_batch.non_tensor_batch["pos_image_id"][0]

            # Extract first sample's prompt
            # For agent loop, raw_prompt contains the prompt messages
            if "raw_prompt" in gen_batch.non_tensor_batch:
                raw_prompt = gen_batch.non_tensor_batch["raw_prompt"][0]
                # Format the prompt messages for readability
                ...
                prompt = "[Prompt not available in raw_prompt]"

            # Decode all rollout responses for first sample
            responses = []
            for i in range(n_rollouts):
                if "responses" in gen_batch_output.batch:
                    response = self.tokenizer.decode(gen_batch_output.batch["responses"][i], skip_special_tokens=True)
                    responses.append(response)

            # Format and save to file
            filename = os.path.join(output_dir, f"step_{step:06d}_rollouts.txt")
            with open(filename, "w", encoding="utf-8") as f:
                f.write(f"=" * 80 + "\n")
                f.write(f"Training Step: {step}\n")
                f.write(f"Number of Rollouts: {n_rollouts}\n")
                f.write(f"=" * 80 + "\n\n")

                # Add metadata section
                f.write(f"METADATA:\n")
                f.write(f"{'-' * 80}\n")
                f.write(f"Dataset Source: {data_source}\n")
                f.write(f"Sample Type: {sample_type}\n")
                f.write(f"Query Base Text: {query_base_text}\n")
                ...
                if pos_video_id:
                    f.write(f"Positive Video ID: {pos_video_id}\n")
                if pos_image_id:
                    f.write(f"Positive Image ID: {pos_image_id}\n")
                f.write(f"{'-' * 80}\n\n")
                ...
                for idx, response in enumerate(responses, 1):
                    f.write(f"ROLLOUT {idx}/{n_rollouts}:\n")
                    f.write(f"{'-' * 80}\n")
                    f.write(f"{response}\n")
                    f.write(f"{'-' * 80}\n\n")

            # Only print on first few steps to avoid log spam
            if step <= 5 or step % 100 == 0:
                print(f"[Rollout Visualization] Saved step {step} rollouts to {filename}")

        except Exception as e:
            # Don't crash training if visualization fails
            import traceback
            print(f"[Warning] Failed to save rollout visualization for step {step}: {e}")
            traceback.print_exc()
            ...
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("gen", timing_raw, color="red"):
                        ...
                        # Save first sample's rollout results for visualization
                        self._save_rollout_visualization(gen_batch_output, gen_batch, self.global_steps)
