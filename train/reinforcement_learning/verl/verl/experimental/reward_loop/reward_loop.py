...
class RewardLoopWorker:
    ...
    async def compute_score_batch(self, data: DataProto) -> list[dict]:
        ...
        logger.warning(f"[RewardLoopWorker.compute_score_batch] Called with data length={len(data)}")

        # Check if reward manager supports batch processing
        if hasattr(self.reward_loop, 'run_batch'):
            logger.warning(f"[RewardLoopWorker.compute_score_batch] Using run_batch")
            # Use batch processing for in-batch negative sampling
            return await self.reward_loop.run_batch(data)
        else:
            logger.warning(f"[RewardLoopWorker.compute_score_batch] run_batch not found, falling back to single-sample processing")
...
class RewardLoopManager:
    ...
    def compute_rm_score(self, data: DataProto) -> DataProto:
        ...
        # DEBUG: Log incoming batch size
        print(f"\n{'='*80}")
        print(f"[RewardLoopManager.compute_rm_score] Received data with len={len(data)}, num_workers={len(self.reward_loop_workers)}")
        print(f"{'='*80}\n")

        chunks = data.chunk(len(self.reward_loop_workers))
        print(f"[RewardLoopManager.compute_rm_score] After chunking: {[len(c) for c in chunks]}")
        outputs = ray.get(
            [
